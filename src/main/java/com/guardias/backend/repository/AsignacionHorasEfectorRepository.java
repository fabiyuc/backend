...
public interface AsignacionHorasEfectorRepository extends JpaRepository<AsignacionHorasEfector, Long> {
       ...
       // ¿Ya existe una asignación de ESTE efector que se solape con el rango
       // recibido?
       @Query("SELECT a FROM asignacionesHorasEfector a " +
                     ...
                     "AND (a.fechaFinalizacion IS NULL OR a.fechaFinalizacion >= :fechaInicio)")
       List<AsignacionHorasEfector> findSolapadasMismoEfector(@Param("idLegajo") Long idLegajo,
                     @Param("idEfector") Long idEfector,
                     @Param("fechaInicio") LocalDate fechaInicio,
                     @Param("fechaFin") LocalDate fechaFin);

       // Asignaciones activas de CUALQUIER efector del legajo que se solapen con el
       // rango recibido
       @Query("SELECT a FROM asignacionesHorasEfector a " +
