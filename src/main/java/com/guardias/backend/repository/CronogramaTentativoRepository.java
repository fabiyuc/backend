...
public interface CronogramaTentativoRepository extends JpaRepository<CronogramaTentativo, Long> {
    ...
    @Query("SELECT ct FROM cronogramasTentativos ct WHERE ct.fechaIngreso = :fecha AND ct.activo = true")
    List<CronogramaTentativo> findByFechaIngresoAndActivoTrue(@Param("fecha") LocalDate fecha);

    @Query("""
                SELECT ct
                FROM cronogramasTentativos ct
                WHERE ct.fechaIngreso BETWEEN :fechaInicio AND :fechaEgreso
                  AND ct.activo = true
            """)
    List<CronogramaTentativo> findByFechaIngresoBetweenAndActivoTrue(
            @Param("fechaInicio") LocalDate fechaInicio,
    ...
    @Query("SELECT ct FROM cronogramasTentativos ct " +
            "WHERE ct.asistencial.id = :idPersona AND ct.efector.id = :idEfector " +
            "AND ct.activo = true " +
            "AND ct.fechaIngreso BETWEEN :fechaInicio AND :fechaFin")
    List<CronogramaTentativo> findActivosPorPersonaEfectorYRango(@Param("idPersona") Long idPersona,
            @Param("idEfector") Long idEfector,
            @Param("fechaInicio") LocalDate fechaInicio,
